    ...
    <div className="home-screen">
      <nav className="home-nav">
        ...
        <ul className={`nav-menu ${navOpen ? 'active' : ''}`}>
          <li><a href="#evenements">8 Mars</a></li>
      ...
      <section className="hero-section" id="hero">
        ...
        <div className="hero-content">
          <p className="hero-eyebrow">bienvenu a la celebration de</p>
          <h1 className="hero-title">La Journée Internationale de Droits de la Femme</h1>
          {/* No subtitle, remove extra text */}
      ...
      <section className="spotlight-section" id="spotlight-grid">
        <div className="section-heading">
          <h2>En Vedette</h2>
          <p>Une sélection de nos étoiles</p>
        </div>
        <div className="spotlight-grid">
          {spotlightModels.slice(0, 23).map((name, index) => (
            <SpotlightCard
              key={modelImages[index % modelImages.length]}
              name={name}
              subtitle={t('labels.featuredRole')}
              imageUrl={modelImages[index % modelImages.length]}
            />
          ))}
        </div>
      </section>
      {/* Removed 'Nos Inspirations' section */}
      ...
      <section className="carousel-section" id="createurs">
        <VideoCarousel
          title="Spectacles Inédits"
          subtitle="Glamour, Danses et Acrobaties"
          videos={creatorsVideos}
        />
      </section>
      <section className="carousel-section" id="talents">
        <VideoCarousel
          title="La Mode au Féminin"
          subtitle={t('sections.talentsSubtitle')}
          videos={talentsVideos}
        />
      </section>
      {/* Removed 'Femmes d\'hier, d\'aujourd\'hui et de demain' section */}
    </div>
  );
}
